MCLExtention='I60'#change the MCLExtention if you want to process file created with another inflation value
...
FastaFileDb='TrimmedPiProt20131014.fasta' ##CHANGE THE DB
CD=os.getcwd() #get current directory and uses it to create file path later
MCL='MCL_Output' #name of the folder containing the MCL_Output
Dump='dump'#name of the folder containing the dump files
MCLDumpDir=os.path.join(CD, MCL, Dump) #create a path to access to the dump file generated via MCL
...
def MCLRatio(MCLDumpDir, MCLExtention, Db_Id_List_Folder, outfile):
    ...
    os.chdir(MCLDumpDir) #set the folder containing the MCL dump file as a wd
    ##--For each file, check protein association in the same clusters and count it--#
    for index, file in enumerate(os.listdir(MCLDumpDir)):
                ...
                for pair in AssoList:
                    if pair not in DoneList or pair[::-1] not in DoneList:
                        if pair in AssoDic or pair[::-1] in AssoDic:
                            AssoDic[pair]=AssoDic.get((coord1, coord2))+1
                        else:
                            AssoDic[(coord1, coord2)]=1
    ...
    for DB_Id_List_File in os.listdir(Db_Id_List_Folder): 
        if DB_Id_List_File.split('_')[1:3] in TreatedFile: #unique identifier Might need removal to make the function more universal --==[[]
            FH=open(DB_Id_List_File, 'r')
            OpenIdList=FH.read().strip()
            for Pair in AssoDic.iterkeys():
                if Pair[0] in OpenIdList and Pair[1] in OpenIdList: #might have a probleme here with the format of readlines
